...
namespace Naver.Compass.Common.CommonBase
{
    ...
    public static class HtmlFromXamlConverter
    {
        ...
        internal static string GetHtmlFromXaml(string xamlString)
        {
            XmlTextReader xamlReader;
            StringBuilder htmlStringBuilder;
            XmlTextWriter htmlWriter;
            ...
            if (!WritePragraphToDiv(xamlReader, htmlWriter))
            {
                return "";
            }
        ...
        public static string TransformXamlToHtml(string xamlString)
        {
            Object obj = XamlReader.Parse(xamlString);
            ...
            if (obj is Section)
            {
                TranslateSectionToHtml(obj as Section,htmlWriter);
            }
            else
            {
                NLogger.Error("Translate to html error,obj is not section.");
        ...
        private static void TranslateSectionToHtml(Section section,XmlTextWriter htmlWriter)
        {
            ...
            foreach (Block block in section.Blocks)
            {
                ...
                {
                    TranslateParagraphToHtml(block as Paragraph, htmlWriter);
                }
                else
                {
                    NLogger.Error("TranslateSectionToHtml=>Block is not paragraph!");
                }
            }
        }

        private static void TranslateParagraphToHtml(Paragraph para, XmlTextWriter htmlWriter)
        {
            htmlWriter.WriteStartElement("P");

            //string stext = "white-space:pre-wrap;";
            string stext = "";
            if (para.Inlines.Count > 0)
            {
                double MLineSpace = 0.00;// para.Inlines.FirstInline.FontFamily.LineSpacing;
                double MFontSize = 0.00;
                foreach (Inline line in para.Inlines)
                {
                    MLineSpace += line.FontFamily.LineSpacing;
                    MFontSize += line.FontSize;
                }

                MLineSpace = MLineSpace / para.Inlines.Count;
                MFontSize = MFontSize / para.Inlines.Count;

                //if (para.LineHeight.CompareTo(Double.NaN) != 0)
                {
                  if (MFontSize < 16)
                  {
                      stext += "line-Height:" + (MLineSpace * (1.355*MFontSize/16 )).ToString();
                  }
                  else
                  {
                      stext += "line-Height:" + (MLineSpace * 1.355).ToString();
                  }
           ...
           htmlWriter.WriteAttributeString("STYLE", stext);
           
            foreach (Inline line in para.Inlines)
            {
                if (line is Run)
                {
                    TranslateRunToHtml(line as Run, htmlWriter);
                }
                else
                {
                    NLogger.Error("TranslateParagraphToHtml=>Line is not run!");
                }
            }
         ...
         private static void TranslateRunToHtml(Run run, XmlTextWriter htmlWriter)
        {
            string sStyle = "";
            htmlWriter.WriteStartElement("Span");

            sStyle += "font-family:" + run.FontFamily.ToString() + ";";
            sStyle += "font-size:" + run.FontSize.ToString() + "px;";
           // sStyle += "vertical-align:-webkit-baseline-middle;";

             if (run.FontStyle != FontStyles.Normal)
             {
                 sStyle += "font-style:" + run.FontStyle.ToString().ToLower() + ";";
             }

             if (run.FontWeight != FontWeights.Normal)
             {
                 sStyle += "font-weight:" + run.FontWeight.ToString().ToLower() + ";";
             }
             sStyle += "color:" + XamlColorToRGBA(run.Foreground.ToString()) + ";";

             string sDecoration = "text-decoration:";
             int index = 0;
             foreach (TextDecoration item in run.TextDecorations)
             {
                 index++;
                 if (item.Location == TextDecorationLocation.Underline)
                 {
                     sDecoration += " underline";
                 }
                 else if (item.Location == TextDecorationLocation.Strikethrough)
                 {
                     sDecoration += " line-through";
                 }
             }
             ...
             htmlWriter.WriteAttributeString("STYLE", sStyle);

             string sText = run.Text.Replace(@"  ", " &nbsp;");
            // string sText = System.Text.RegularExpressions.Regex.Replace(run.Text, @" ", @"&nbsp;");
             //sText = sText.Replace(@"\", @"\\");
             ...
             int index2 = sText.IndexOf(@" ");
             if (index2 >= 0)
             {
                 htmlWriter.WriteString(sText);
             }
             else
             {
                 htmlWriter.WriteString(sText);
             }
             //htmlWriter.WriteWhitespace(sText);
        ...
        private static bool WritePragraphToDiv(XmlTextReader xamlReader, XmlTextWriter htmlWriter)
        {
            if (!ReadNextToken(xamlReader))
            {
                // Xaml content is empty - nothing to convert
            ...
            if ((xamlReader.Name == "FlowDocument" || xamlReader.Name == "Section" || xamlReader.Name== "Span"))
            {
                // Create a buffer StringBuilder for collecting css properties for inline STYLE attributes
                // on every element level (it will be re-initialized on every level).
                StringBuilder inlineStyle = new StringBuilder();
            ...
            while (xamlReader.MoveToNextAttribute())
            {
                ...
                switch (xamlReader.Name)
                {
                    ...
                    case "TextDecorations":
                        css = "text-decoration:"+ParseTextDecoration(xamlReader.Value);
                        break;
                    case "TextEffects":
                        break;
                    case "Emphasis":
                    ...
                    case "BorderBrush":
                        ...
                        break;
                    case "LineHeight":
                        css = "line-Height:" + xamlReader.Value +"px;";
                        break;
                    case "TextIndent":
        ...
        private static string XamlColorToRGBA(string color)
        {
            if (color.StartsWith("#"))
            {
                if (color.StartsWith("#"))
                ...
                string value = "rgba(";
                value += Convert.ToByte((v >> 16) & 255).ToString();
                value += ",";
                value += Convert.ToByte((v >> 8) & 255).ToString();
                value += ",";
                value += Convert.ToByte((v >> 0) & 255).ToString();
                value += ",";
                value += Math.Round(Convert.ToDouble((v >> 24) & 255)/255,2).ToString();
                value += ")";
        ...
        private static string ParseTextDecoration(string sDecoration)
        {
            if (sDecoration.CompareTo("Underline") == 0)
            {
                return "underline";
            }
            else if (sDecoration.CompareTo("Strikethrough") == 0)
            {
                return "line-through";
            }
            else
        ...
        private static void WriteElementContent(XmlTextReader xamlReader, XmlTextWriter htmlWriter, StringBuilder inlineStyle)
        {
            ...
            else
            {
                ...
                if (xamlReader.Name == "Run" && (!elementContentStarted))
                {
                    htmlWriter.WriteString("\x01");
                }
        ...
        private static void AddComplexProperty(XmlTextReader xamlReader, StringBuilder inlineStyle)
        {
            ...
            {
                inlineStyle.Append("text-decoration:underline line-through");
            }
