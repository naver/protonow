...
namespace Xceed.Wpf.AvalonDock
{
    ...
    public class DockingManager : Control, IOverlayWindowHost, INotifyPropertyChanged//, ILogicalChildrenContainer
    {
        ...
        public DockingManager()
        {
            ...
            //load current language.
             ...
             {
                 LoadLanguage.GetCulture();
             }
             HidePaneCommand = new RelayCommand((p) => ExecuteHideSideCommand(p), (p) => CanExecuteHideSideCommand(p));
        }
        ...
        #region CanRelocateWindow
        public static readonly DependencyProperty CanRelocateWindowProperty =
        DependencyProperty.Register("CanRelocateWindow", typeof(bool), typeof(DockingManager),
        new FrameworkPropertyMetadata(null));

        /// <summary>
        /// If support relocate window position by 
        /// dragging and drop window in overlay window.
        /// </summary>
        public bool CanRelocateWindow
        {
            get { return (bool)GetValue(CanRelocateWindowProperty); }
            set { SetValue(CanRelocateWindowProperty, value); }
        }
        #endregion
        ...
        #region Hide/Show left/right side panels

        /// <summary>
        /// DockCommand Dependency Property
        /// </summary>
        public static readonly DependencyProperty HidePaneCommandProperty =
            DependencyProperty.Register("HidePaneCommand", typeof(ICommand), typeof(DockingManager),
                new FrameworkPropertyMetadata(null,
        ...
        public ICommand HidePaneCommand
        {
            get { return (ICommand)GetValue(HidePaneCommandProperty); }
            set { SetValue(HidePaneCommandProperty, value); }
        }
        ...
        {
            ((DockingManager)d).OnDockCommandChanged(e);
        }
        ...
        {
           // bool boo = RightSidePanel.IsVisible;
        }

        private bool CanExecuteHideSideCommand(object parameter)
        {
            return true;
        }

        /// <summary>
        /// Hide/Show one whole side
        /// </summary>
        /// <param name="parameter">whick side</param>
        private void ExecuteHideSideCommand(object parameter)
        {
            var paneGroups = Layout.RootPanel.Children.OfType<LayoutAnchorablePaneGroup>();
            if (paneGroups == null)
            {
                return;
            }
            AnchorSide hideSide;
            LayoutAnchorablePaneGroup paneGroup = null;
            switch (parameter.ToString())
            {
                case "Left":
                    hideSide = AnchorSide.Left;
                    paneGroup = paneGroups.FirstOrDefault(a => a.Orientation == Orientation.Vertical);
                    break;
                case "Right":
                    hideSide = AnchorSide.Right;
                    paneGroup = paneGroups.LastOrDefault(a => a.Orientation == Orientation.Vertical);
                    break;
                //case "Top":
                //    hideSide = AnchorSide.Top;
                //    break;
                //case "Bottom":
                //    hideSide = AnchorSide.Bottom;
                //    break;
                default:
                    hideSide = AnchorSide.None;
                    break;
            }
            if(paneGroup == null)
            {
            ...
            paneGroup.Hidden = !paneGroup.Hidden;


            if (false == paneGroup.Hidden)
            {
                var panes = paneGroup.Children.FirstOrDefault(a => a.IsVisible);
                if (panes == null)
                {
                    foreach (LayoutAnchorablePane pane in paneGroup.Children)
                    {
                        var panel = _layoutItems.FirstOrDefault(a => (a.LayoutElement as LayoutAnchorable).ContentId == pane.Name);
                        (panel.LayoutElement as LayoutAnchorable).Show();
                    }
                }

                foreach (LayoutAnchorableFloatingWindow floatingWindow in Layout.FloatingWindows)
                {
                    var item = floatingWindow.RootPanel.Descendents().OfType<LayoutAnchorable>().Where(a => a.DockSide == hideSide);

                    if (item.Count() != 0)
                    {
                        floatingWindow.RootPanel.Hidden = !floatingWindow.RootPanel.Hidden;
                    }
                }
            }
            else
        ...
        public bool IsLeftPanesVisible
        {
            get
            {
                var paneGroups = Layout.RootPanel.Children.OfType<LayoutAnchorablePaneGroup>();
                if (paneGroups != null)
                {
                    var paneGroup = paneGroups.FirstOrDefault(a => a.Orientation == Orientation.Vertical);
                    if (paneGroup != null)
                    {
                        var visbleChild = paneGroup.Children.FirstOrDefault(a => a.IsVisible);
                        if (visbleChild == null)
                        {
                            paneGroup.Hidden = true;
                        }
                        if (!paneGroup.Hidden)
                            return true;
                    }
        ...
        public bool IsRightPanesVisible
        {
            get
            {
                var paneGroups = Layout.RootPanel.Children.OfType<LayoutAnchorablePaneGroup>();
                if (paneGroups != null)
                {
                    var paneGroup = paneGroups.LastOrDefault(a => a.Orientation == Orientation.Vertical);
                    if (paneGroup != null)
                    {
                        var visbleChild = paneGroup.Children.FirstOrDefault(a => a.IsVisible);
                        if (visbleChild == null)
        ...
        public void FirePaneVisible()
        {
            RaisePropertyChanging("IsLeftPanesVisible");
            RaisePropertyChanging("IsRightPanesVisible");
        }
        ...
        void DockingManager_Loaded(object sender, RoutedEventArgs e)
        {
            if (!DesignerProperties.GetIsInDesignMode(this))
            {
                ...
                if (IsVisible && CanRelocateWindow)
                    CreateOverlayWindow();
        ...
        protected virtual void OnThemeChanged(DependencyPropertyChangedEventArgs e)
        {
            ...
            if (newTheme != null)
            {
                ...
                {
                    //NLogger.Warn("OnThemeChanged failed: " + ex.Message);
                    return;
                }
        ...
        public static readonly DependencyProperty GridSplitterWidthProperty =
            DependencyProperty.Register("GridSplitterWidth", typeof(double), typeof(DockingManager),
                new FrameworkPropertyMetadata((double)4.0));
