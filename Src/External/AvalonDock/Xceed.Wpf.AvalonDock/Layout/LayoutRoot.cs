        ...
        public void CollectGarbage()
        {
            ...
            do
            {
                ...
                foreach (var emptyPane in this.Descendents().OfType<ILayoutPane>().Where(p => p.ChildrenCount == 0))
                {
                    ...
                    //...if this pane has been marked as not garbage collectible then skip it
                    if (!emptyPane.GarbageCollectible)
                    {
                        emptyPane.ComputeVisibility();
                        continue;
                    }
                ...
                if (!exitFlag)
                {
                    ...
                    foreach (var emptyPaneGroup in this.Descendents().OfType<LayoutAnchorablePaneGroup>().Where(p => p.ChildrenCount == 0))
                    {
                        ...
                        if (!emptyPaneGroup.GarbageCollectible)
                        {
                            emptyPaneGroup.ComputeVisibility();
                            continue;
                        }
            ...
            System.Diagnostics.Debug.Assert(
                !this.Descendents().OfType<LayoutAnchorablePane>().Any(a => a.ChildrenCount == 0 && a.IsVisible && a.GarbageCollectible));
#if TRACE
